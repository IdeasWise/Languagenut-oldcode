...
$(document).ready(function(){
	...
	$('#gamedata_language_uid').bind({ change : function() {
		this.form.submit();
	}});
	
	$('.activate-school-package').bind(
		{ click : function() {				
			return confirm('Has payment been received for this package?');				

		} 
	});
	$('.remove-class-package').bind(
		{ click : function() {				
			return confirm('Are you sure want to remove this package from this class?');				
	...
	$checkAll.live('click', function() {
		var $checkbox_class = ($(this).attr('id')).replace('checkall_','checkbox_');
		$('input.'+$checkbox_class).attr('checked',true);
		return false;
	});
	
	$unCheckAll = $('a.uncheckall');
	$unCheckAll.live('click', function() {
		var $checkbox_class = ($(this).attr('id')).replace('uncheckall_','checkbox_');
		$('input.'+$checkbox_class).attr('checked',false);
		return false;
	});
	
	$("ul.subnav").parent().append("<span></span>"); //Only shows drop down trigger when js is enabled - Adds empty span tag after ul.subnav
	
	$("ul.topnav").find("li a").not('.direct').click(function() {
		if($(this).attr("href") == "#") {
			return false;	
		}
	});
	...
	$('ul.topnav li').each(function(){
		...
		if($this.parent().hasClass('topnav')) {
			$this.find('a:eq(0)').not('.direct').click(function(){ //When trigger is clicked...
	...
	$add_student_rows = $(".inputClass").length;
	$add_student_html = '<tr>';
	$add_student_html += '<td></td>';
	$add_student_html += '<td><input type="text" name="last[]" class="box"></td>';
	$add_student_html += '<td><input type="text" name="first[]" class="box"></td>';
	$add_student_html += '<td></td>';
	$add_student_html += '</tr>';
	$add_student_button = $('#add_student_submit');
	$input_limit = 60;
	$student_count = $('#student_count');
	if($student_count.val() > 0) {
		$input_limit = $input_limit - ($student_count.val()*2);
	}
	
	$('#add_student_button').bind( 
	{ 
		click : function ()
		{
			$add_student_table = $('#GavScore tr:last');
			if( $add_student_rows < $input_limit) {
				$add_student_button.attr('name', 'add_students');
				$add_student_rows +=2;
				$add_student_table.after($add_student_html);
			}
		}
	} );
	
	$add_template_html = '<p>'+$('#templage_uid_html').html()+'</p>';
	$template_container = $('#template_container');
	$add_template = $('#add_template');
	$add_template.live('click', function() {
		$template_container.append($add_template_html);
	});
	
	$multiple_page_radio = $('#multiple_page_radio');
	$multiple_page_radio.live('click', function() {
		if($(this).attr("checked")) {
			$add_template.show();
		}		
	});
	
	$single_page_radio = $('#single_page_radio');
	$single_page_radio.live('click', function() {
		if($(this).attr("checked")) {
			$add_template.hide();
	...
	$('.view-game-scores').bind( {
						   	click : function(){
								window.location = $(this).attr("name");
								} 
						   } );
	$('#vocab_language_id').bind(
				{ change : function() {
				this.form.submit();
				}});
	
	$('.Ajax-delete-page').bind(
				{ click : function() {				
	...
	$('.button-paid').bind({
						   click: function () {
							   ...
							   var PaidUid = 0;
							   PaidUid = $(this).attr("id").replace("button-","");
							  $section = window.location.toString();
							  if( $section.match('/account/') ) {
								  $section = 'account';
							  } else {
								   $section = 'admin';
							  }
							   ...
							   if(confirm($(this).attr("title"))){
								$.ajax({
								   type: "POST",
								   url: url+$section+"/invoice/school/paid",
	...
	$('.create-profile-button').bind(
				{ click : function() {
				
					var FormObj = this.form;
					window.location = FormObj.ProfileLink.value.replace('ProfileType',FormObj.user_type.value);
					return false;
   ...
   /* $('label.listbox').each(function(){
	...
	$('.get_students').click(function(){
									//  alert(this.href.replace('#','').replace('edit','getstudentsbyname'));
									  AjaxUpdateDiv(this.href.replace('#','').replace('edit','getstudentsbyname'),'add_students');
									  
									  return false;
									  });
	
	
	$('.add-students').live('click', function() {
  // Live handler called.
 // alert(this.checked);
	...
	if( $(this).attr('alt') != 1)
 	...
 	if(this.checked){
		 ...
		 var actionurl = url+sap+'/classes/addstudent/'+this.value+'/'+$('#class_uid').val();
		 AjaxUpdateDiv(actionurl, 'current_student');
	}
	else{
		var actionurl = url+sap+'/classes/removestudent/'+this.value+'/'+$('#class_uid').val();
		if(confirm( 'Are you sure you want to remove '+this.title+' from this class?'))
		 	AjaxUpdateDiv(actionurl, 'current_student');
		else
			this.checked = true;
	}
...
function AjaxUpdateDiv(queryString, ContainerId)
{
		$.ajax({
               type: "POST",
               url: queryString,               
...
function OnlyAjaxCall(queryString, wantAlert)
{
		$.ajax({
               type: "POST",
               url: queryString,               
               success: function(msg){
                   if( msg.trim() != '' && wantAlert)
                 alert( msg.trim() );
               }
             });
}
...
function f_processcsv(){
	document.getElementById('h_processcsv').value='1';
