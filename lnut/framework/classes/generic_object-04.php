	...
	*/
	public function isValidarrFields($arrFields=array(),$objTable=false){
		...
		foreach($arrFields as $index => $arrInfo ) {
			...
			} else if($arrInfo['minChar'] > 0 && $arrInfo['maxChar'] > 0 && (strlen($arrInfo['value']) < $arrInfo['minChar'] || strlen($arrInfo['value']) > $arrInfo['maxChar']) ) {
				$arrMessages[$arrInfo['errIndex']] = $arrInfo['errMinMax'];
			} else if($arrInfo['maxChar'] > 0 && strlen($arrInfo['value']) > $arrInfo['maxChar'] ) {
				$arrMessages[$arrInfo['errIndex']] = $arrInfo['errMinMax'];
