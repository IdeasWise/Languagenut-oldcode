...
class page_subscribe_homeuser_stage_4_translations extends generic_object {
	...
	private function isValidateFormData() {
		...
		$arrFields = array(
			...
			'subject' => array(
				'value' => (isset($_POST['subject'])) ? trim($_POST['subject']) : '',
				'checkEmpty' => false,
				'errEmpty' => '',
				'minChar' => 5,
				'maxChar' => 64,
				'errMinMax' => 'Subject must be 5 to 255 characters in length.',
				'dataType' => 'text',
				'errdataType' => 'Please enter valid subject.',
				'errIndex' => 'error.subject'
			),
			'body' => array(
				'value' => (isset($_POST['body'])) ? trim($_POST['body']) : '',
				'checkEmpty' => true,
				'errEmpty' => 'Please enter body text.',
				'minChar' => 0,
				'maxChar' => 0,
				'errMinMax' => '',
				'dataType' => 'text',
				'errdataType' => 'Please enter valid body text.',
				'errIndex' => 'error.body'
			)
		);
		...
		if ($this->isValidarrFields($arrFields, $this) === true) {
			...
			$this->set_subject($arrFields['subject']['value']);
			$this->set_body($arrFields['body']['value']);
			return true;
		} else {
			return false;
		}
