...
class page_subscribe_school_stage_1_translations extends generic_object {
	...
	public function subscribeValidation($redirect='subscribe/school/') {
		...
		$form1 = array(
			...
			'reseller_code_uid' => array('value' => '', 'error' => false)
			)
		);
		if (count($_POST) > 0) {
			/**
			...
			$reseller_code_uid = (isset($_POST['reseller_code_uid']) && strlen((int)$_POST['reseller_code_uid']) > 0) ? (int)$_POST['reseller_code_uid'] : '';
...
//				$message.= '<p><strong><a href="#" class="errorClose" style="text-decoration:none;font-size:15px;font-family:Arial;">'.config::translate('form.back').'</a></strong></p>';
	...
	public function getSchoolPackage($locale='en') {
		$arrPackage = array();
		$sql = "SELECT * FROM `reseller_sub_package` rsp
				WHERE
				rsp.reseller_uid IN
					(SELECT pr.iuser_uid FROM `profile_reseller` pr
					WHERE
					pr.locale_rights='{$locale}')
				AND
				rsp.is_default_school_package='1'
				";
		$arrPackage = database::arrQuery($sql);
		return (isset($arrPackage[0])) ? $arrPackage[0] : array();
	}
