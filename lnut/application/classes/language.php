...
class language extends generic_object {
	...
	public function isValidateFormData() {
		...
		$show_reseller_codes = (isset($_POST['show_reseller_codes'])) ? (int)$_POST['show_reseller_codes'] : '';
		...
		if (count($arrMessages) == 0) {
			...
			$this->set_show_reseller_codes($show_reseller_codes);
		...
		if ($result && mysql_error() == '' && mysql_num_rows($result)) {
			while ($row = mysql_fetch_assoc($result)) {
				...
				$arrResponse[$row['uid']] = array(
					...
					'available' => ($support_language_uid==107&&$row['uid']==16)?1:$row['available'],
					'runtime' => stripslashes($row['runtime']),
					'audiodirectory' => stripslashes($row['audiodirectory']),
					'is_learnable' => ($row['is_learnable'] == 1) ? true : (($support_language_uid==107 && $row['uid']==16)?true:false),
