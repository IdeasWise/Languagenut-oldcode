...
class user extends generic_object {
	...
	protected function isValidData($update = false) {
		...
		if(!$error) {
			...
			if($password != "") {
				$this->arrFields['password']['Value']   =   $password;
	...
	public function userRedirectUrl() {
            ...
            if(@$_SESSION['user']['admin'] == 1)
                return "admin";
            elseif(is_array(@$_SESSION['user']['user_type']) ){
	...
	public function isValidLogin() {
		...
		if(is_numeric($user_uid) && $user_uid > 0) {
			...
			if(md5($_POST['password']) != $oldPassword) {
				$response['message']    =   "The details you have entered do not match our records.1";
			} else if($this->get_access_allowed() == 0) {
				$response['message']    =   "The details you have entered do not match our records.2";
			}
			if($this->get_deleted() == 1) {
				$response['message']    =   "The details you have entered do not match our records.3";
			}
        ...
        public function has_active_subscription()
        {
            ...
            if( strtotime($subObj->TableData['due_date']['Value']) > time())
                $return =  true;
            if( strtotime($subObj->TableData['expires_dts']['Value']) > time() && $subObj->TableData['verified']['Value'] == 1)
                $return = true;
