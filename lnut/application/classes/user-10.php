...
class user extends generic_object {
	...
	public function get_users($pageId = '',$all = false) { 
		...
		if(@$_SESSION['user']['admin'] != 1 && $where == ''){
			$where = " where 1 = 1 ";
			if($_SESSION['user']['userRights'] == 'school'){
				$where .= " AND ( `uid` IN (SELECT `iuser_uid` FROM profile_schooladmin 
					  WHERE school_id = '".@$_SESSION['user']['school_uid']."') 
					  OR
					  `uid` IN (SELECT `iuser_uid` FROM profile_schoolteacher 
					  WHERE school_id = '".@$_SESSION['user']['school_uid']."') 
					  OR
					  `uid` IN (SELECT `iuser_uid` FROM profile_student 
					  WHERE school_id = '".@$_SESSION['user']['school_uid']."')
				   )";
			}

			if($_SESSION['user']['userRights'] == 'schooladmin'){
				$where .= " AND ( `uid` IN (SELECT `iuser_uid` FROM profile_schoolteacher 
					  WHERE school_id = '".@$_SESSION['user']['school_uid']."') 
					  OR
					  `uid` IN (SELECT `iuser_uid` FROM profile_student 
					  WHERE school_id = '".@$_SESSION['user']['school_uid']."')
			...
			if($_SESSION['user']['userRights'] == 'schoolteacher'){
				$where .= " AND ( `uid` IN (SELECT `iuser_uid` FROM profile_student 
					  WHERE school_id = '".@$_SESSION['user']['school_uid']."')
				   )";
			}
		}
		...
		if($all == false) {
			 $sql = 'SELECT COUNT(`uid`) FROM `user` '.$where;

			if(@$_SESSION['user']['admin'] != 1)
			{
				if(strtolower( @$parts[2] ) == 'schooladmin'){


					 $sql = "SELECT COUNT(U.`uid`) FROM user U,profile_schooladmin P where FIND_IN_SET('schooladmin',`user_type`)
							AND P.iuser_uid = U.uid and P.school_id = '".@$_SESSION['user']['school_uid']."'";
				}

				if(strtolower( @$parts[2] ) == 'schoolteacher'){


					 $sql = "SELECT COUNT(U.`uid`) FROM user U,profile_schoolteacher P where FIND_IN_SET('schoolteacher',`user_type`)
							AND P.iuser_uid = U.uid and P.school_id = '".@$_SESSION['user']['school_uid']."'";
							
				}

				if(strtolower( @$parts[2] ) == 'student'){


					 $sql = "SELECT COUNT(U.`uid`) FROM user U,profile_student P where FIND_IN_SET('student',`user_type`)
							AND P.iuser_uid = U.uid and P.school_id = '".@$_SESSION['user']['school_uid']."'";
			...
			if(@$_SESSION['user']['admin'] != 1)
			{
				if(strtolower( @$parts[2] ) == 'schooladmin'){
					 ...
					 $sql = "SELECT U.* FROM user U,profile_schooladmin P where FIND_IN_SET('schooladmin',`user_type`)
							AND P.iuser_uid = U.uid and P.school_id = '".@$_SESSION['user']['school_uid']."'
							ORDER BY `registered_dts` DESC LIMIT ".$this->get_limit();
				}

				if(strtolower( @$parts[2] ) == 'schoolteacher'){


					 $sql = "SELECT U.* FROM user U,profile_schoolteacher P where FIND_IN_SET('schoolteacher',`user_type`)
							AND P.iuser_uid = U.uid and P.school_id = '".@$_SESSION['user']['school_uid']."'
							ORDER BY `registered_dts` DESC LIMIT ".$this->get_limit();
				}
				...
				if(strtolower( @$parts[2] ) == 'student'){
					 ...
					 $sql = "SELECT U.* FROM user U,profile_student P where FIND_IN_SET('student',`user_type`)
							AND P.iuser_uid = U.uid and P.school_id = '".@$_SESSION['user']['school_uid']."'
							ORDER BY `registered_dts` DESC LIMIT ".$this->get_limit();
				}
			}
	...
	public function userRedirectUrl() {
		if(@$_SESSION['user']['admin'] == 1) {
			return "admin";
		} else if(is_array(@$_SESSION['user']['user_type']) ) {
	...
	public function has_active_subscription() {
		...
		$subObj->load(array(), array('user_uid'=> $user_uid));

		$now = time();

		if( strtotime($subObj->TableData['due_date']['Value']) > $now) {
			$return =  true;
		}
		//if( strtotime($subObj->TableData['expires_dts']['Value']) > $now && $subObj->TableData['verified']['Value'] == 1) {
		if( strtotime($subObj->TableData['expires_dts']['Value']) > $now) { // fix all verified dates
			$return = true;
		}
	...
	public function getSchoolId() {
		...
		if( is_array( $userType ) ){
			if( in_array( 'school', $userType ) ) {
				...
				$sql = "SELECT S.user_uid FROM profile_schooladmin T, users_schools S WHERE T.school_id = S.uid and T.iuser_uid = '".$this->get_uid()."' LIMIT 0,1";
				$result = database::query($sql);
				if(mysql_num_rows($result)){
				   ...
				   $sql = "SELECT S.user_uid FROM profile_schoolteacher T, users_schools S WHERE T.school_id = S.uid and T.iuser_uid = '".$this->get_uid()."' LIMIT 0,1";
				   $result = database::query($sql);
				   if(mysql_num_rows($result)){
					   $row = mysql_fetch_array($result);
					   return $row['user_uid'];
			   ...
			   elseif( in_array( 'homeuser', $userType ) ){
				   return $this->get_uid();
			   }
			}
			return 0;
	 ...
	 public function getSchoolIdForAccount()
	{
		...
		if( is_array( $userType ) ){
			   if( in_array( 'school', $userType ) ){
				   $sql = "SELECT uid FROM users_schools where user_uid = '".$this->get_uid()."' LIMIT 0,1";
				   $result = database::query($sql);
				   if(mysql_num_rows($result)){
					   $row = mysql_fetch_array($result);
					   return $row['uid'];
			   ...
			   elseif( in_array( 'schooladmin', $userType ) ){
				   $sql = "SELECT school_id FROM profile_schooladmin where iuser_uid = '".$this->get_uid()."' LIMIT 0,1";
				   $result = database::query($sql);
				   if(mysql_num_rows($result)){
					   $row = mysql_fetch_array($result);
			   ...
			   elseif( in_array( 'schoolteacher', $userType ) ){
				   $sql = "SELECT school_id FROM profile_schoolteacher where iuser_uid = '".$this->get_uid()."' LIMIT 0,1";
				   $result = database::query($sql);
				   if(mysql_num_rows($result)){
					   $row = mysql_fetch_array($result);
					   return $row['user_uid'];
	...
	public function updateUsername(){
		...
		$open_username = substr($firstname,0,1).substr($lastname,0,1).rand(0,10).rand(0,10).rand(0,10).rand(0,10);
