...
class schooladmin_package extends generic_object {
    ...
    public function saveSections() {
        ...
        if (isset($_POST['submit'])) {
            $query = "DELETE ";
            $query.="FROM ";
            $query.="`schooladmin_package_sections` ";
            $query.="WHERE ";
            $query.="`package_uid` IN(";
            $query.="SELECT uid FROM";
            $query.=" `schooladmin_package`";
            $query.=" WHERE";
            $query.=" uid='{$_POST['package_uid']}'";
            $query.=" AND school_uid='{$school_uid}'";
            $query.=")";

            database::query($query);
            $objPackage = new schooladmin_package($_POST['package_uid']);
            $objPackage->load();
            ...
            foreach ($_POST['section'] as $index => $value) {
                ...
                $sql = "INSERT INTO `schooladmin_package_sections`";
                $sql.=" SET ";
    ...
    public function deleteBeforeUpdate($schoolUid=0, $packageUid=0) {
        $sql = "UPDATE schooladmin_package SET
					deleted='1'
					WHERE 
    ...
    public function isValidCreate($schoolUid, $packageUid) {
        ...
        $sql = "SELECT uid FROM schooladmin_package
					WHERE
                    iupdated_date='0' 
                    AND
                    school_uid='{$schoolUid}'
        ...
        foreach ($packageRecord as $record) {
            $sql = "UPDATE schooladmin_package SET
                ...
                ";
            database::query($sql);
            $sql = "INSERT INTO schooladmin_package SET
        ...
        foreach ($package_activity_Record as $record) {
            ...
            $values = trim($values, ",");
            $sql = "INSERT INTO schooladmin_package_activity SET
        ...
        foreach ($package_activity_readingRecord as $record) {
            ...
            $values = trim($values, ",");
            $sql = "INSERT INTO schooladmin_package_activity_reading SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
            database::insert($sql);
        ...
        foreach ($package_activity_speaklistenRecord as $record) {
            ...
            $sql = "INSERT INTO schooladmin_package_activity_speaklisten SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
            database::insert($sql);
        ...
        foreach ($package_activity_writingRecord as $record) {
            ...
            $sql = "INSERT INTO schooladmin_package_activity_writing SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
            database::insert($sql);
        ...
        foreach ($package_gamesRecord as $record) {
            ...
            $values = trim($values, ",");
            $sql = "INSERT INTO schooladmin_package_games SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
            database::insert($sql);
        ...
        foreach ($package_languageRecord as $record) {
            ...
            $sql = "INSERT INTO schooladmin_package_language SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
            database::insert($sql);
        ...
        foreach ($package_priceRecord as $record) {
            ...
            $sql = "INSERT INTO schooladmin_package_price SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
            database::insert($sql);
        ...
        foreach ($package_sectionsRecord as $record) {
            ...
            $sql = "INSERT INTO schooladmin_package_sections SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
            database::insert($sql);
        ...
        foreach ($package_translationRecord as $record) {
            ...
            $sql = "INSERT INTO schooladmin_package_translation SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
            database::insert($sql);
        ...
        foreach ($package_unitsRecord as $record) {
            ...
            $sql = "INSERT INTO schooladmin_package_units SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
            database::insert($sql);
        ...
        foreach ($package_yearsRecord as $record) {
            ...
            $sql = "INSERT INTO schooladmin_package_years SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
            database::insert($sql);
    ...
    public function isValidUpdate($uid, $schoolUid, $resellerPackageUid, $resellerSubPackageUid, $arrData=array()) {
        ...
        if ($this->isValidateFormData()) {
            ...
            $sql.=" school_package_uid='{$newPackageUid}'";
            $sql.=" WHERE";
            $sql.=" school_package_uid='{$uid}'";
            database::query($sql);
            // end for school package
            $sql = "SELECT * FROM schooladmin_package_activity_reading WHERE package_uid='{$uid}'";
            $package_activity_readingRecord = database::arrQuery($sql);
            foreach ($package_activity_readingRecord as $record) {
                $values = "";
                foreach ($record as $key => $value) {
            ...
            $sql = "SELECT * FROM schooladmin_package_activity_speaklisten WHERE package_uid='{$uid}'";
            $package_activity_speaklistenRecord = database::arrQuery($sql);
            foreach ($package_activity_speaklistenRecord as $record) {
                $values = "";
                foreach ($record as $key => $value) {
            ...
            $sql = "SELECT * FROM schooladmin_package_activity_writing WHERE package_uid='{$uid}'";
            $package_activity_writingRecord = database::arrQuery($sql);
            foreach ($package_activity_writingRecord as $record) {
                $values = "";
                foreach ($record as $key => $value) {
            ...
            $sql = "SELECT * FROM schooladmin_package_games WHERE package_uid='{$uid}'";
            $package_gamesRecord = database::arrQuery($sql);
            foreach ($package_gamesRecord as $record) {
                $values = "";
                foreach ($record as $key => $value) {
            ...
            $sql = "SELECT * FROM schooladmin_package_sections WHERE package_uid='{$uid}'";
            $package_sectionsRecord = database::arrQuery($sql);
            foreach ($package_sectionsRecord as $record) {
                $values = "";
                foreach ($record as $key => $value) {
            ...
            $sql = "SELECT * FROM schooladmin_package_translation WHERE package_uid='{$uid}'";
            $package_translationRecord = database::arrQuery($sql);
            foreach ($package_translationRecord as $record) {
                $values = "";
                foreach ($record as $key => $value) {
            ...
            $sql = "SELECT * FROM schooladmin_package_units WHERE package_uid='{$uid}'";
            $package_unitsRecord = database::arrQuery($sql);
            foreach ($package_unitsRecord as $record) {
                $values = "";
                foreach ($record as $key => $value) {
            ...
            $sql = "SELECT * FROM schooladmin_package_years WHERE package_uid='{$uid}'";
            $package_yearsRecord = database::arrQuery($sql);
            foreach ($package_yearsRecord as $record) {
                $values = "";
                foreach ($record as $key => $value) {
            ...
            if ($newPackageUid != null && isset($_POST['learnable_language_uid']) && count($_POST['learnable_language_uid'])) {
                foreach ($_POST['learnable_language_uid'] as $learnable_language_uid) {
                    $sql = "INSERT INTO `schooladmin_package_language`";
                    $sql.= " SET ";
            ...
            if ($newPackageUid != null && isset($_POST['price']) && is_array($_POST['price'])) {
                foreach ($_POST['price'] as $index => $value) {
                    ...
                    if (mysql_error() == '' && mysql_num_rows($result)) {
                        ...
                        parent::__construct($row['uid'], 'schooladmin_package_price');
                        $this->load();
                        ...
                        $sql = "INSERT INTO schooladmin_package_price ";
                        $sql.=" SET";
                        $sql.=" package_uid='{$newPackageUid}',";
                        $sql.=" price='{$value}',";
                        if (isset($_POST['vat'][$index])) {
...
//		$response = false;
//
//		$sql = "SELECT * FROM `schooladmin_package_translation`
//		WHERE
//		schooladmin_package_uid='{$uid}'";
...
//	}
    ...
    public function getLearnableLanguages($packageUid) {
        ...
        $sql.=" `schooladmin_package_language`";
        $sql.=" WHERE ";
