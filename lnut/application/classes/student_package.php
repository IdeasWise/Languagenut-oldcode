...
class student_package extends generic_object {
	...
	public function saveSections() {
		...
		if (isset($_POST['submit'])) {
			$query = "DELETE ";
			$query.="FROM ";
			$query.="`student_package_sections` ";
			$query.="WHERE ";
			...
			foreach ($_POST['section'] as $index => $value) {
				...
				$sql = "INSERT INTO `student_package_sections`";
				$sql.=" SET ";
	...
	public function isValidUpdate($uid, $studentUid, $classPackageUid, $arrData=array()) {

		if ($this->isValidateFormData()) {
			$this->load();

			$sql = "UPDATE `student_package` SET ";
			$sql.=" updated_date='" . date("Y-m-d H-i-s") . "',";
			$sql.=" iupdated_date='" . time() . "'";
			$sql.=" WHERE uid='{$uid}'";
			database::query($sql);
			$sql = "INSERT INTO `student_package` SET
					student_uid='{$studentUid}',
					class_package_uid='{$classPackageUid}',					
					name='{$_POST["name"]}',
			...
			$sql = "SELECT * FROM student_package_activity_reading WHERE package_uid='{$uid}'";
			$package_activity_readingRecord = database::arrQuery($sql);
			foreach ($package_activity_readingRecord as $record) {
				$values = "";
				foreach ($record as $key => $value) {
				...
				$values = trim($values, ",");
				$sql = "INSERT INTO student_package_activity_reading SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
				database::insert($sql);
			}
			$sql = "SELECT * FROM student_package_activity_speaklisten WHERE package_uid='{$uid}'";
			$package_activity_speaklistenRecord = database::arrQuery($sql);
			foreach ($package_activity_speaklistenRecord as $record) {
				$values = "";
				foreach ($record as $key => $value) {
				...
				$values = trim($values, ",");
				$sql = "INSERT INTO student_package_activity_speaklisten SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
				database::insert($sql);
			}
			$sql = "SELECT * FROM student_package_activity_writing WHERE package_uid='{$uid}'";
			$package_activity_writingRecord = database::arrQuery($sql);
			foreach ($package_activity_writingRecord as $record) {
				$values = "";
				foreach ($record as $key => $value) {
				...
				$values = trim($values, ",");
				$sql = "INSERT INTO student_package_activity_writing SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
				database::insert($sql);
			}
			$sql = "SELECT * FROM student_package_games WHERE package_uid='{$uid}'";
			$package_gamesRecord = database::arrQuery($sql);
			foreach ($package_gamesRecord as $record) {
				$values = "";
				foreach ($record as $key => $value) {
				...
				$values = trim($values, ",");
				$sql = "INSERT INTO student_package_games SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
				database::insert($sql);
			}
			$sql = "SELECT * FROM student_package_sections WHERE package_uid='{$uid}'";
			$package_sectionsRecord = database::arrQuery($sql);
			foreach ($package_sectionsRecord as $record) {
				$values = "";
				foreach ($record as $key => $value) {
				...
				$values = trim($values, ",");
				$sql = "INSERT INTO student_package_sections SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
				database::insert($sql);
			}
			$sql = "SELECT * FROM student_package_translation WHERE package_uid='{$uid}'";
			$package_translationRecord = database::arrQuery($sql);
			foreach ($package_translationRecord as $record) {
				$values = "";
				foreach ($record as $key => $value) {
				...
				$values = trim($values, ",");
				$sql = "INSERT INTO student_package_translation SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
				database::insert($sql);
			}
			$sql = "SELECT * FROM student_package_units WHERE package_uid='{$uid}'";
			$package_unitsRecord = database::arrQuery($sql);
			foreach ($package_unitsRecord as $record) {
				$values = "";
				foreach ($record as $key => $value) {
				...
				$values = trim($values, ",");
				$sql = "INSERT INTO student_package_units SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
				database::insert($sql);
			}
			$sql = "SELECT * FROM student_package_years WHERE package_uid='{$uid}'";
			$package_yearsRecord = database::arrQuery($sql);
			foreach ($package_yearsRecord as $record) {
				$values = "";
				foreach ($record as $key => $value) {
				...
				$values = trim($values, ",");
				$sql = "INSERT INTO student_package_years SET
                    package_uid='{$newPackageUid}',
                    {$values}
                ";
				database::insert($sql);
			...
			if ($newPackageUid != null && isset($_POST['learnable_language_uid']) && count($_POST['learnable_language_uid'])) {
				foreach ($_POST['learnable_language_uid'] as $learnable_language_uid) {
					$sql = "INSERT INTO `student_package_language`";
					$sql.= " SET ";
			...
			if ($newPackageUid != null && isset($_POST['price']) && is_array($_POST['price'])) {
				foreach ($_POST['price'] as $index => $value) {
					...
					if (mysql_error() == '' && mysql_num_rows($result)) {
						...
						parent::__construct($row['uid'], 'student_package_price');
						$this->load();
						...
						$sql = "INSERT INTO student_package_price ";
						$sql.=" SET";
						$sql.=" package_uid='{$newPackageUid}',";
						$sql.=" price='{$value}',";
						if (isset($_POST['vat'][$index])) {
	...
	public function getClassPackage($class_uid=0) {
		if ($class_uid > 0) {
			$query = "SELECT * FROM ";
			$query.=" `class_package`";
			$query.=" WHERE ";
