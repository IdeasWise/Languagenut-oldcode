...
class user extends generic_object {
	...
	private function getAllUsersList( $all = false, $parts = array() ) {
		...
		if($all	== false) {
			...
			$query.= "`user` as `U` ".$where." ";
			$query.= "ORDER BY ";
			$query.= "`U`.`registered_dts` DESC ";
			$query.= "LIMIT ".$this->get_limit();
			return database::arrQuery($query);
		} else {
			$query = "SELECT * FROM `user` as `U`	".$where." ORDER BY	`U`.`registered_dts` DESC";
			return database::arrQuery($query);
		}
	}
	...
	private function getSchoolList( $all = false, $parts = array() ) {
		...
		if($all	== false) {
			...
			$query = "SELECT * FROM `user` AS `U`, `users_schools` AS `SC`	".$where." ORDER BY	`U`.`registered_dts` DESC";
			return database::arrQuery($query);
		}		
	}
	...
	private function getSchoolAdminList( $all = false, $parts = array() ) {
		...
		if($all	== false) {
			...
			$query = "SELECT * FROM `user` AS `U`, `profile_schooladmin` AS `SA`	".$where." ORDER BY	`U`.`registered_dts` DESC";
			return database::arrQuery($query);
		}		
	}
	...
	private function getSchoolTeacherList( $all = false, $parts = array() ) {
		...
		if($all	== false) {
			...
			$query = "SELECT * FROM `user` AS `U`, `profile_schoolteacher` AS `ST`	".$where." ORDER BY	`U`.`registered_dts` DESC";
			return database::arrQuery($query);
		}		
	}
	...
	private function getSchoolStudentList( $all = false, $parts = array() ) {
		...
		if($all	== false) {
			...
			$query = "SELECT * FROM `user` AS `U`, `profile_student` AS `ST`	".$where." ORDER BY	`U`.`registered_dts` DESC";
			return database::arrQuery($query);
		}		
	}
	...
	private function getHomeuserList( $all = false, $parts = array() ) {
		...
		if($all	== false) {
			...
			$query = "SELECT * FROM `user` AS `U`, `profile_homeuser` AS `H`	".$where." ORDER BY	`U`.`registered_dts` DESC";
			return database::arrQuery($query);
		}		
	}
	...
	public function	isValidLogin() {
		...
		if(validation::isPresent('password',$_POST)) {
			...
			$response['fields']['password']['error']		= true;
			$response['fields']['password']['highlight']	= true;
			$response['message']	=	"Password is requried.";
		}
		if(is_numeric($user_uid) &&	$user_uid >	0) {
