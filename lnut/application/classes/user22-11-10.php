	...
	public function get_users($pageId = '',$all = false) { 
		...
		if($all == false) {
			...
			if(isset($_SESSION['user']['admin']) && $_SESSION['user']['admin'] != 1) {
				...
				if(strtolower( @$parts[2] ) == 'schoolteacher' && isset($_SESSION['user']['school_uid'])) {
					$sql = "SELECT ";
					$sql.= "COUNT(`U`.`uid`) ";
					$sql.= "FROM ";
					$sql.= "`user` AS `U`,";
				...
				if(strtolower( @$parts[2] ) == 'student' && isset($_SESSION['user']['school_uid'])) {
					$sql = "SELECT ";
					$sql.= "COUNT(`U`.`uid`) ";
					$sql.= "FROM ";
					$sql.= "`user` AS `U`,";
			...
			if($_SESSION['user']['admin'] != 1) {
				if(strtolower( @$parts[2] ) == 'schooladmin' && isset($_SESSION['user']['school_uid'])) {
					$sql = "SELECT ";
					$sql.= "`U`.* ";
					$sql.= "FROM ";
					$sql.= "`user` AS `U`,";
