...
class profile_translator extends generic_object {
	...
	public function doSave () {
		...
		if( $this->isValidateFormData() == true ) {
			...
			if(isset($_POST['uid']) && is_numeric($_POST['uid']) && $_POST['uid'] > 0) {
				...
				$query.="`user` ";
				$query.="SET ";
				$query.="`user_type` = CONCAT(`user_type` , ',translator') ";
				$query.="WHERE ";
	...
	private function isValidateFormData() {
		...
		$locale_rights		= (isset($_POST['locale_rights']) && count($_POST['locale_rights'])) ? implode(',',$_POST['locale_rights']) : ''; 
		...
		if( trim(strlen($vfax)) < 9 || trim(strlen($vfax)) > 21 ) {
			$arrMessages['error_vfax'] = "Fax number must be 9 to 21 characters in length.";
		} else if(!validation::isValid('fax',$vfax) ) {
			$arrMessages['error_vfax'] = "Please enter valid fax number.";
		}

		if( trim(strlen($vphone)) < 9 || trim(strlen($vphone)) > 21 ) {
			$arrMessages['error_vphone'] = "Phone number must be 9 to 21 characters in length.";
