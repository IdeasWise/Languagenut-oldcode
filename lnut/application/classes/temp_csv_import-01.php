	...
	public function isValidTranslationSubmission() {
		if(isset($_POST['submit'])) {
			 ...
			 if(isset($_POST['language_uid']) && trim($_POST['language_uid']) == '0' ) {
				 $message['error.language_uid'] = "Please select a language.";
			 }
			 if(!isset($_FILES['csv_file'])) {
				 $message['error.csv_file'] = "Please upload translation csv file.";
			 }
			 if(isset($_FILES['csv_file']) && !$this->isValidCsvfile()) {
				 $message['error.csv_file'] = "Please upload valid translation csv file.";
			 }			 
		}
		$response	= array();
		$msg		= NULL;
		
		foreach( $message as $idx => $val ){
