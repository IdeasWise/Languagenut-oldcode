	...
	private function isValidateFormData() {
		...
		$arrFields = array(
			...
			'call_status' => array(
				...
				'minChar' => 0,
				'maxChar' => 1,
				'errMinMax' => 'Please select call status.',
				'dataType' => 'int',
				'errdataType' => 'Please enter valid call status.',
				'errIndex' => 'error_call_status'
			)
		);
		...
		if ($this->isValidarrFields($arrFields, $this) === true) {
			...
			if($objUser->get_valid()) {
				...
				$objUser->set_call_status($arrFields['call_status']['value']);
				$objUser->save();
			}
	...
	private function sendRegistrationMail($user_uid, $school_uid) {
		...
		if (is_array($data) && count($data)) {
			...
			$this->mail_html(
				...
				'',
				'',
				'cshepherd@languagenut.com,support@languagenut.com',
				''
			);
		}
	}
