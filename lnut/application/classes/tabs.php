...
class tabs {
	...
	public function GetArticleTabs($index=null,$tableName=null,$tabName=null,$XHTML=null,$article_uid=null,$objArticle=false)
	{
		if($index== null || $tableName==null || $tabName==null || $XHTML==null || $article_uid==null ) {
			return array('none'=>'');
		}
		...
		if(isset($_POST['form_submit_language'])){
			if(isset($_POST['language_uid']) && !empty($_POST['language_uid'])){
				$query ="INSERT INTO ";
				$query.=mysql_real_escape_string($_POST['table_name']). " ";
				$query.="SET ";
				$query.="language_uid = '".mysql_real_escape_string($_POST['language_uid'])."',";
		...
		$query .= "`language` AS `LG` ";
		$query .= "WHERE ";
		$query .= "`LG`.`uid` = `TB`.`language_uid` ";
		$query .= "AND ";
		$query .= "`TB`.`article_uid`= '".mysql_real_escape_string($article_uid)."'";
		$query .= "GROUP BY ";
		$query .= "`TB`.`uid` ";
		$query .= "ORDER BY ";
		$query .= "`LG`.`name` ";
		...
		if($result && mysql_error()=='' && mysql_num_rows($result) > 0) {
			while($row=mysql_fetch_assoc($result)) {
				...
				foreach( $row as $idx => $val ) {
					$row[$idx.'.id'] = $row['language_uid'].$idx.'.id'.$tabName;
				}

				$row['cms.id']		= $row['language_uid'].'.cms'.$tabName;
				$row['table_name']	= $tableName;
				$row['tabName']		= $tabName;
				$row['action']		= $_SERVER['REQUEST_URI'];
		...
		if(count($data) > 1) {
			...
			$row['table_name'] = $tableName;
			$row['action'] = $_SERVER['REQUEST_URI'];
			$row['locale'] = format::to_select(array("name" => 'language_uid',"id" => 'language_uid',"options_only" => false), $data , NULL);
			$panel->assign($row);
			$panel->assign($arrLanguage);
		...
		$page_title = "<a href='".config::url('admin/article/list/')."'>Article List<a> &gt; ";
		if($objArticle->get_title() !='' ) {
			$page_title .= "<a href='".config::url('admin/article/edit/'.$objArticle->get_uid().'/')."'>".$objArticle->get_title()."</a> &gt; ";
		}
		$page_title .= "Content";
		$body->assign(
			array(
