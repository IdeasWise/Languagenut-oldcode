<?php

/**
 * flashdev.php
 */

class FlashDev extends Controller {

	public function __construct () {
		parent::__construct();

		$this->page();
	}

	protected function page () {
            
		$test = new subscriptions();
		$test->load();

		if(count($_SESSION) > 0 && isset($_SESSION['user']) && isset($_SESSION['user']['logged_in']) && $_SESSION['user']['logged_in']==true) {
			$user_uid	= $_SESSION['user']['uid'];
			//$type		= $_SESSION['user']['type'];
			$locale		= $_SESSION['user']['prefix'];
			$locale = config::get('locale');

			$support_language_id = 14;
			
			$query = "SELECT `uid` from `language` WHERE `prefix` = '".$locale."'";
			$result = database::query($query);
			if( $result && mysql_num_rows($result) ){
				$row = mysql_fetch_array($result);
				$support_language_id = $row['uid'];
			}

			if(isset($_GET['support']) && (int)$_GET['support'] > 0) {
				$support_language_id = $_GET['support'];
			}


			$validate = array();
			$type = array('school', 'schooladmin', 'schoolteacher', 'student', 'homeuser');
			$validate = array_intersect(@$_SESSION['user']['user_type'], $type);
                      
			//if($type=='user' || $type=='school') {
			if( count($validate)){
				$paths = config::get('paths');

				if(count($paths) > 2) {
					
				} else {
					/**
					 * Fetch the flash public xhtml page template
					 */
					$skeleton = new xhtml ('skeleton.flashdev');
					$skeleton->load();
					$skeleton->assign(
						array(
							'translate:need_flash'			=> config::translate('need_flash'),
							'support_language_id'					=> $support_language_id
						)
					);

					/**
					 * Fetch the page details
					 */
					$page = new page('index');

					/**
					 * Build the output
					 */
					$skeleton->assign (
						array (
							'title'			=> $page->title(),
							'keywords'		=> $page->keywords(),
							'description'	=> $page->description()
						)
					);

					output::as_html($skeleton,true);
				}
			} else {
				// do nothing yet
				//print_r($_SESSION);
			}
		} else {
			//print_r($_SESSION);
			output::redirect(config::url('logout/'));
		}
	}
}

?>