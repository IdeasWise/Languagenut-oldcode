...
class account_users extends Controller {
	...
	public function __construct() {
		...
		$token = (isset($this->arrPaths[2]) ? $this->arrPaths[2] : '');
		$profile = (isset($this->arrPaths[3]) ? $this->arrPaths[3] : '');

		if ('' != $token && in_array($token, $this->arrTokens)) {
			$this->token = str_replace(array('user', '-'), array('', ''), $token);
		}

		if ($profile != '' && isset($this->arrPaths[4]) && $token == 'profile') {
			if (in_array(strtolower($profile), $this->arrProfiles)) {
				$objUser = new user($this->arrPaths[4]);
				if ($objUser->get_valid()) {
					// validate user
					// if yes then load controller
					// other wise redirect him to list page
					$controller = 'admin.users.profile.' . strtolower($profile);
	...
	protected function doAdd() {
		...
		$skeleton		= config::getUserSkeleton();
		$objLanguage	= new language();
		$body			= make::tpl('body.account.schooladmin.users.add');
		$action			= "Add";
		$body->assign("action", $action);
