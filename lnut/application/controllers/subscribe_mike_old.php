...
class Subscribe extends Controller {
	...
	public function __construct() {
		...
		if (isset($paths[1]) && $paths[1] == 'cybersource') {

			$this->doCybersource();
			exit;
		}
		if (isset($this->locale) && strlen($this->locale) > 0) {
			if (isset($paths[1]) && in_array($paths[1], array('homeuser', 'school', 'schoolsubscribe'))) {
				$this->type = $paths[1];
				$this->typegiven = true;
			}
			...
			output::redirect(config::url('subscribe_mike/'));
		}
	}
...
//				'locale'		=> $this->locale
	...
	protected function show_school() {
		 ...
		 */
		$stage = 1;
		if (isset($_SESSION['stage']) && in_array($_SESSION['stage'], array(1, 2, 3, 4, 'school_package', 'paymentoption'))) {
...
//		list($arrErrors, $arrMessage, $form1) = $objSchoolStage1->subscribeValidation();
		...
		if (count($arrStageInfo) > 0) {
			...
			$reseller_code_uids = '';
//			$show_reseller_codes = false; // not in ell
			...
			if($result && mysql_error()=='' && mysql_num_rows($result) > 0) {
				...
				if($row['show_reseller_codes']==1) {
					...
					if($result && mysql_error()=='' && mysql_num_rows($result) > 0) {
//						$show_reseller_codes = true; // not in ell
...
//			}
			...
			$body = new xhtml('body.subscribe.school.stages.mike');
			$body->load();
			$body->assign(
				array(
					'errors'								=> $arrMessage,
					...
					'signtype'								=> $signType
					// 'reseller_code_visibility'				=> $reseller_visibility // not in ell
				)
			);
...
//			$label_school_postcode = stripslashes($arrStageInfo['label_school_postcode']);
			 ...
			 * Fetch the body content
			 */
			$body= make::tpl ('body.subscribe.school.stage2.mike');
			$body->assign(
				array(
					'errors'					=> $arrMessage,
					'type'						=> 'school',
...
//					'translate.label_whole_school_password'	=> $label_whole_school_password
		...
		if (count($form1) > 0 && count($form2) > 0 && count($form3) > 0) {
			...
			if ($user_uid > 0) {
				...
				if($school_uid > 0) {
					...
					$_SESSION['login_password'] = (isset($form1['password']['value'])) ? $form1['password']['value'] : "";

					/**
					* Get the default product for this locale
					*/
					$default_product_array = product_locale::getDefaultByLocale();
					$product_uid = $default_product_array['product_uid'];
					$product_locale_uid = $default_product_array['uid'];
					$price = $default_product_array['years_1'];
					...
					if ($subscribe_uid > 0) {
						...
						// create in the products table an entry based on subscription id
						if(false !== ($uid = subscriptions_products::addEntry($subscribe_uid,$product_locale_uid,$product_uid))) {
	
							// if we are good to go continue sending mails and allowing login
							if(false !== ($success=subscriptions::ProcessSchoolProductPackages($subscribe_uid,$user_uid))) {
								
								// send welcome mail to school, moved in here from ell
								...
								if (isset($_SESSION['cybersource_status']))
									unset($_SESSION['cybersource_status']);
								...
								*/
								...
								// ProcessSchoolProductPackages failed
							}
						} else {
							// subscriptions_products::addEntry failed
							mail('mike@notonlybutalso.net', 'LanguageNut Error - school subscription failed', mysql_error() . $query, 'From: errors@languagenut.com');
						}
					} else {
						// CreateSchoolSubscription failed
						mail('mike@notonlybutalso.net', 'LanguageNut Error - school subscription failed', mysql_error() . $query, 'From: errors@languagenut.com');
					}
				} else {
					// SubscribeSchoolSave failed
					mail('mike@notonlybutalso.net', 'LanguageNut Error - school id not created', mysql_error() . $query, 'From: errors@languagenut.com');
				}
			} else {
				// SubscribeSave failed
				mail('mike@notonlybutalso.net', 'LanguageNut Error - user id not created', mysql_error() . $query, 'From: errors@languagenut.com');
			}
		} // form errors end
	...
	protected function show_school_stage_school_package() {
		...
		$objSchoolStage1 = new page_subscribe_school_stage_1_translations();
		$objSchoolStage1->load(array(), array('locale' => $this->locale));
		$arrPackage = $objSchoolStage1->getSchoolPackage($this->locale);
		if (count($arrPackage) == 0) {
			...
			output::redirect(config::url("subscribe/"));
			exit;
		}
		$arrStageInfo = array();
		...
		if (count($arrStageInfo) > 0) {
			...
			$body = new xhtml('body.subscribe.school.stages_package');
			$body->load();

			$arrBodyTranslation = array();
			$arrTranslations = page_subscribe_payment::getByLanguageName($this->locale);
			foreach ($arrTranslations as $arrTranslation) {
				$arrBodyTranslation[$arrTranslation["translation_key"]] = $arrTranslation["translation_value"];
			}
			$body->assign($arrBodyTranslation);
			...
			$ComboExpirationMonth = format::to_select($attributes, $months);
			...
			$ComboExpirationYear = format::to_select($attributes, $options);
			$body->assign(
					array(
						'errors' => $arrMessage,
						'type' => 'cybersource',
						'signtype' => $signType,
						'unitPrice' => $arrPackage["price"] + $arrPackage["vat"],
						'packageUid' => $arrPackage["uid"],
						'ComboExpirationMonth' => $ComboExpirationMonth,
						'ComboExpirationYear' => $ComboExpirationYear,
					)
			);
			if (count($_POST) > 0) {
				$body->assign($_POST);
	...
	protected function show_school_stage_paymentoption() {
		$page = new page('subscribe');
		$body = make::tpl('body.subscribe.school.paymentoption');
		$body->assign('pay_by_invoice',config::translate('pay_by_invoice'));
		$body->assign('pay_by_cybersource',config::translate('pay_by_cybersource'));
	...
	protected function show_school_stage_school_payment() {
		/**
		 * Check SESSION variables
		 */
		$query = '';
