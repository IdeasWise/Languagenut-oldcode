...
class account_schooladmin_package extends Controller {
	...
	protected function doCancelrequest() {
		if(isset($this->arrPaths[3]) && is_numeric($this->arrPaths[3])) {
			...
			$objSchoolPackages->CancelPurchaseRequest($this->arrPaths[3],$this->reseller_uid);
			output::redirect(config::url('account/order-packages/pending-orders/'));
		}
	}

	protected function doMakeabuyrequest() {
		if(isset($this->arrPaths[3]) && is_numeric($this->arrPaths[3])) {
			$objSchoolPackages = new school_packages();
			$objSchoolPackages->MakeABuyRequest($this->arrPaths[3],$this->reseller_uid);
	...
	protected function doActivepackages() {
		...
		$body = make::tpl('body.account.schooladmin.active.package.list');
		$objSchoolPackages = new school_packages();
		...
		if ($arrPackages && count($arrPackages) > 0) {
			...
			foreach ($arrPackages as $uid => $arrData) {
				$row = make::tpl('body.account.schooladmin.active.package.list.row')->assign($arrData);
	...
	protected function doPendingorders() {
		$skeleton = config::getUserSkeleton();
		$body = make::tpl('body.account.schooladmin.pending.order.list');
		$objSchoolPackages = new school_packages();
		$arrPackages = $objSchoolPackages->getListPendingPackageOrders($this->reseller_uid);

		if ($arrPackages && count($arrPackages) > 0) {
			$rows = array();
			foreach ($arrPackages as $uid => $arrData) {
				$row = make::tpl('body.account.schooladmin.pending.order.list.row')->assign($arrData);
				$rows[] = $row->get_content();
			}
			$body->assign('rows', implode('', $rows));
		}
