...
class controller_classes extends Controller {
	...
	protected function doAdd() {
		...
		if (isset($_POST['form_submit_button'])) {
			...
			if ($objClasses->doSave()) {
				...
				$school_id=(isset($objClasses->arrForm['school_id']))?$objClasses->arrForm['school_id']:'';
				$objClasses->arrForm['school_id'] = $objSchool->SchoolListBox('school_id', $school_id);
				$body->assign($objClasses->arrForm);
			}
		} else {
			$arrBody['school_id'] = $objSchool->SchoolListBox('school_id', null);
		}

		$body->assign($arrBody);
		$body->assign('breadcrumb', '<p class="breadcrumb"><a href="' . config::admin_uri() . 'classes/">Classes</a> &gt; Add Class</p>');
		$skeleton->assign(
		array(
		'body' => $body
		)
	...
	protected function breadCrumb($class_uid, $class_name) {
		if (isset($_SESSION['user']['admin']) && $_SESSION['user']['admin'] == 1) {
			$html = '<a href="' . config::admin_uri() . 'classes/">Classes List</a> &gt; <a href="' . config::admin_uri() . 'classes/edit/' . $class_uid . '/">' . $class_name . '</a>';
		} else {
			$html = '<a href="' . config::admin_uri() . 'classes/">My Classes</a> &gt; <a href="' . config::admin_uri() . 'classes/edit/' . $class_uid . '/">' . $class_name . '</a>';
		}
		return $html;
	}
		...
		if (isset($this->arrPaths[3]) && $this->arrPaths[3] > 0) {
			...
			if (!empty($arrStudents)) {
				...
				if(isset($_SESSION['user']['userRights']) && $_SESSION['user']['userRights']=='schoolteacher') {
					$template = 'body.account.schoolteacher.class.students.row';
				}
				foreach ($arrStudents as $uid => $data) {
	...
	*/
	...
	protected function doList() {
		...
		$arrRows = array();
		$template = 'body.account.classes.list.row';
		if(isset($_SESSION['user']['userRights']) && $_SESSION['user']['userRights']=='schoolteacher') {
			$template = 'body.account.schoolteacher.classes.list.row';
		}
		if (!empty($arrClasses)) {
			foreach ($arrClasses as $uid => $arrClass) {

				$arrRows[] = make::tpl($template)->assign($arrClass)->get_content();
			}
		}
