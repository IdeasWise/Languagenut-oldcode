...
class admin_article_pages extends Controller {
	...
	protected function doList() {
		$skeleton = make::tpl('skeleton.admin');
		$body = make::tpl('body.admin.article.page.list');
		$objArticlePage = new article_page();
		$arrGroups = $objArticlePage->getList($this->article_uid);

		if (count($arrGroups) > 0) {
			$rows = array();
			foreach ($arrGroups as $uid => $arrData) {
				$rows[] = make::tpl('body.admin.article.page.list.row')->assign($arrData)->get_content();
			}

			$page_display_title = $objArticlePage->get_page_title('Page {CURRENT} of {MAX}<br />Displaying results {FROM} to {TO} of {TOTAL}');

			$page_navigation = $objArticlePage->get_prev('<a href="{LINK_HREF}">{LINK_LINK}</a>');
			$page_navigation .= $objArticlePage->get_range('<a href="{LINK_HREF}">{LINK_LINK}</a>', ' &raquo ');
			$page_navigation .= $objArticlePage->get_next('<a href="{LINK_HREF}">{LINK_LINK}</a>');
