...
class admin_article_template_content_translation extends Controller {
	...
	protected function doGroups() {
		...
		$template_content_translation_uid = (isset($this->arrPaths[5]) && is_numeric($this->arrPaths[5]))?$this->arrPaths[5]:null;

		$objTemplateGroupContent	= new template_group_content_translations();
		$arrGroups	= $objTemplateGroupContent->getContentGroupList($this->template_translation_uid,$template_content_translation_uid);
	...
	protected function doAdd() {
		$skeleton	= make::tpl('skeleton.admin');
		$body		= make::tpl('body.admin.article.template.content.translation.add');
		$support_language_uid	= 0;
		$arrLearnable			= array();
		if(count($_POST) > 0) {
			$objTemplateContent = new template_content_translations();
	...
	protected function doList () {
		$skeleton		= make::tpl('skeleton.admin');
		$body			= make::tpl('body.admin.article.template.content.translation.list');
		$objTemplateContent	= new template_content_translations();
		$arrTemplateContent	= $objTemplateContent->getList($this->template_translation_uid);

		if($arrTemplateContent && count($arrTemplateContent) > 0) {
			$rows = array ();
			foreach($arrTemplateContent as $uid=>$arrData) {
				$rows[] = make::tpl('body.admin.article.template.content.translation.list.row')->assign($arrData)->get_content();
			}
