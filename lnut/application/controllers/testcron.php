		...
		if($result && mysql_error() == '') {
			if(mysql_num_rows($result) > 0) {
				...
				foreach($schools as $index=>$details) {
					if(date('Y-m-d',strtotime($details['end'])) == date('Y-m-d',strtotime('+28 days')) || 1==1) {
					...
					if(date('Y-m-d',strtotime($details['end'])) == date('Y-m-d',strtotime('+14 days')) || 1==1) {
					...
					if(date('Y-m-d',strtotime($details['end'])) == date('Y-m-d',strtotime('+7 days')) || 1==1) {
					...
					if(date('Y-m-d',strtotime($details['end'])) == date('Y-m-d',strtotime('1 day')) || 1==1) {
					...
					if(date('Y-m-d',strtotime($details['end'])) == date('Y-m-d',strtotime('-1 days')) || 1==1) {
//					if($details['end'] - $today >= -86400 && $details['end'] - $today < 0) {
	...
	private function mail_html ($to='',$subject='',$message='',$from='',$receiptname='',$receiptmail='',$cc='',$bcc='') {
		...
		mail('sam@languagenut.com', $subject, $message, $header);
		mail('swyam.joshi@latitudetechnolabs.com', $subject, $message, $header);
	}
}
