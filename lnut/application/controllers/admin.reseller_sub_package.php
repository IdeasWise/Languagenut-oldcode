...
class admin_reseller_sub_package extends Controller {
	...
	protected function verifyParentPackage($reseller_uid=null,$package_uid=null) {
		if($reseller_uid!=null && $package_uid!=null) {
			$this->objResellerPackage = new reseller_package();
			return $this->objResellerPackage->isValidPackage($reseller_uid,$package_uid);
		}
	}
	protected function doAdd() {
		if(isset($this->arrPaths[3]) && isset($this->arrPaths[4]) && is_numeric($this->arrPaths[3]) && is_numeric($this->arrPaths[4]) ) {
			if($this->verifyParentPackage($this->arrPaths[3],$this->arrPaths[4])===true) {

				$skeleton= make::tpl('skeleton.admin');
				$body	 = make::tpl('body.admin.reseller_sub_package.add');

				$objReseller = new profile_reseller ();
				$resellerName = $objReseller->getResellerNameByUid($this->arrPaths[3]);
				...
				if (count($_POST) > 0) {
					...
					if (($response = $objPackage->isValidCreate()) === true) {
						output::redirect(config::url('admin/reseller_sub_package/list/' . $this->arrPaths[3] . '/' . $this->arrPaths[4] . '/'));
					} else {
	...
	protected function doEdit() {
		if(isset($this->arrPaths[4]) && isset($this->arrPaths[5]) && is_numeric($this->arrPaths[4]) && is_numeric($this->arrPaths[5]) ) {
			...
			if($this->verifyParentPackage($this->arrPaths[4],$this->arrPaths[5])===true) {
				$skeleton = make::tpl('skeleton.admin');
				$body = make::tpl('body.admin.reseller_sub_package.edit');
				$uid = $this->arrPaths[3];
				...
				$resellerName = profile_reseller::getResellerNameByUid($this->arrPaths[4]);
				$body->assign('reseller_name', $resellerName);
				$body->assign('uid', $uid);
				$body->assign('reseller_uid',$this->objResellerPackage->get_reseller_uid());
				$body->assign('package_uid', $this->objResellerPackage->get_uid());
	...
	protected function doList() {
		$skeleton = make::tpl('skeleton.admin');
		$body = make::tpl('body.admin.reseller_sub_package.list');
		$objResellerPackage = new reseller_sub_package();
		$arrPackages = $objResellerPackage->getList($this->arrPaths[3],$this->arrPaths[4]);
		...
		if ($arrPackages && count($arrPackages) > 0) {
			...
			foreach ($arrPackages as $uid => $arrData) {
				...
				$row = make::tpl('body.admin.reseller_sub_package.list.row')->assign($arrData);
				$rows[] = $row->get_content();
			}
			$body->assign('rows', implode('', $rows));
		}
		$resellerName = profile_reseller::getResellerNameByUid($this->arrPaths[3]);
		$body->assign('reseller_uid', $this->arrPaths[3]);
		$body->assign('reseller_name', $resellerName);
		$body->assign('package_uid', $this->arrPaths[4]);
		...
		if (isset($this->arrPaths[3]) && is_numeric($this->arrPaths[3])) {
			...
			if($objPackage->get_valid()) {
				...
				if (isset($_POST['submit'])) {
					...
					output::redirect(config::url('admin/reseller_sub_package/sections/' . $this->arrPaths[3] . '/' . $this->arrPaths[4] . '/' . $this->arrPaths[5] . '/'));
				}
					...
					if($this->verifyParentPackage($this->arrPaths[4],$this->arrPaths[5])===true) {
						$skeleton = make::tpl('skeleton.admin');
						$body = make::tpl('body.admin.reseller_sub_package.tab');
						$resellerName = profile_reseller::getResellerNameByUid($this->arrPaths[4]);
						$body->assign('reseller_name', $resellerName);
		...
		if (isset($this->arrPaths[3]) && is_numeric($this->arrPaths[3])) {
			...
			if($objPackage->get_valid()) {
				...
				if (isset($_POST['submit'])) {
					...
					output::redirect(config::url('admin/reseller_sub_package/gamesandactivities/' . $this->arrPaths[3] . '/' . $this->arrPaths[4] . '/' . $this->arrPaths[5] . '/'));
				}
				...
				if(isset($this->arrPaths[4]) && isset($this->arrPaths[5]) && is_numeric($this->arrPaths[4]) && is_numeric($this->arrPaths[5]) ) {
					if($this->verifyParentPackage($this->arrPaths[4],$this->arrPaths[5])===true) {
						$skeleton = make::tpl('skeleton.admin');
						$body = make::tpl('body.admin.reseller_sub_package.gamesandactivities.tabs');
