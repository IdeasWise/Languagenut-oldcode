...
class account_student_package extends Controller {
    ...
    protected function doPackage_edit() {
        ...
        $body = new xhtml('body.account.student_package.edit');
        $body->load();

        $student_uid = 0;
        $class_uid = 0;
        list($student_uid, $class_uid) = explode("_", $this->parts[4]);
        $package_uid = $this->parts[5];
        $objstudentPackage = new student_package($package_uid);

        if ($objstudentPackage->get_valid()) {
            $objstudentPackage->load();
        }


        $body->assign('package_uid', $package_uid);
        $body->assign('student_uid', $student_uid);
        $body->assign('class_uid', $class_uid);

        $support_language_uid = 0;
        $arrLearnable = array();
        if (count($_POST) > 0) {
            $objPackage = new student_package();

            if (($response = $objPackage->isValidUpdate($package_uid, $student_uid, $objstudentPackage->get_class_package_uid())) === true) {
                output::redirect(config::url("account/classes/studentpackage/list/{$student_uid}_{$class_uid}/"));
            } else {
        ...
        $selectedLearnableLanguage = $objstudentPackage->getLearnableLanguages($package_uid);
        $arrLearnableLanguages = $this->getLearnableLanguages($objstudentPackage->get_class_package_uid(), $selectedLearnableLanguage);
        $body->assign(
                array(
                    'name' => $objstudentPackage->get_name()
                )
        );
        $body->assign(
                array(
    ...
    private function getEditPriceForm($packageUid=0) {
        ...
        $query.="`price` ";
        $query.="FROM ";
        $query.="`student_package_price` ";
        $query.="WHERE ";
        $query.="`student_package_price`.`locale` = `language`.`prefix` ";
        $query.="AND ";
        $query.="`student_package_price`.`package_uid`='{$packageUid}' ";
        $query.="LIMIT 1";
        $query.=") AS `price`, ";
    ...
    private function getPackageLearnableLanguages($package_uid=null, $endMethod=null) {
        if ($package_uid != null && $endMethod != null) {
            ...
            $query.=" FROM ";
            $query.=" `language` AS `L`, ";
            $query.=" `student_package_language` AS `PL`";
            $query.=" WHERE ";
    ...
    private function getPackageYears($language_uid=null, $package_uid=null, $endMethod=null) {
        if ($language_uid != null && $package_uid != null && $endMethod != null) {
            ...
            $query.="FROM ";
            $query.="`years` AS `Y`, ";
            $query.="`student_package_sections` AS `PS`";
            $query.=" WHERE ";
    ...
    private function getGames($language_uid=null, $section_uid=null) {
        ...
        if ($language_uid != null && $section_uid != null) {
            $objPackageGames = new student_package_games();

            $objPackageSections = new student_package($this->parts[5]);
            if ($objPackageSections->get_valid()) {
                $objPackageSections->load();
            }
            $class_package_uid = $objPackageSections->get_class_package_uid();

            $packuid = $class_package_uid;

             $objSchool = new class_package($packuid);
            $objSchool->load();
            $schoolpackuid=$objSchool->get_school_package_uid();
            ...
            $query.=" WHERE ";
            $query.=" package_uid = '{$schoolpackuid}'";
    ...
    private function getSections($unit_uid=null, $year_uid=null, $learnable_language_uid=null) {
        ...
        if ($year_uid != null && $unit_uid != null && $learnable_language_uid != null) {
            ...
            $l_uid = $learnable_language_uid;
            $packuid = $objPackageSections->get_class_package_uid();
            $objSchool = new class_package($packuid);
            $objSchool->load();
            $schoolpackuid=$objSchool->get_school_package_uid();
//            $table_name = 'class_package';
            $table_name = 'schooladmin_package';
...
//						";
    ...
    private function getLearnableLanguages($classPackageUid, $arrLearnable = array()) {
        ...
        $query.=" FROM ";
        $query.="`class_package_language` ";
        $query.="WHERE ";
        $query.=" package_uid IN ( ";
        $query.=" SELECT uid FROM";
        $query.=" `class_package`";
        $query.=" WHERE";
        $query.=" uid='{$classPackageUid}'";
        $query.=" ) ";
        $query.=" ) ";
        $query.="ORDER BY `name` ";
    ...
    protected function doPackage_sections() {
        $skeleton = config::getUserSkeleton();
        $body = make::tpl('body.account.student_package.tab');
        ...
        $body->assign('student_name', $objClassesStudent->getStudentNameByUid($student_uid));

        $body->assign('class_name', $objClass->get_name());
        $body->assign('package_uid', $package_uid);

        if (isset($_POST['submit'])) {
            $objPackageSections = new student_package();
        ...
        if (isset($package_uid) && is_numeric($package_uid)) {
            $objPackage = new student_package($package_uid);
            ...
            if ($objPackage->get_valid()) {
                ...
                $query.="`language` AS `L`, ";
                $query.="`student_package_language` AS `PL`,";
                $query.="`student_package` AS `SP`";
                $query.="WHERE ";
                $query.="`PL`.`learnable_language_uid` = `L`.`uid` ";
                $query.=" AND ";
                $query.="`PL`.`package_uid` = '{$package_uid}' ";
                $query.=" AND ";
                $query.="`SP`.`student_uid` = '{$student_uid}' ";
                $query.=" AND ";
                ...
                output::redirect(config::url('account/student_package/'));
            }
        } else {
            output::redirect(config::url('account/student_package/'));
        }
    ...
    protected function doPackage_gamesandactivities() {
        $skeleton = config::getUserSkeleton();
        $body = make::tpl('body.account.student_package.gamesandactivities.tabs');
        $student_uid = 0;
        $class_uid = 0;
        list($student_uid, $class_uid) = explode("_", $this->parts[4]);
        $package_uid = $this->parts[5];
        ...
        if (isset($package_uid) && is_numeric($package_uid)) {
            ...
            if ($objPackage->get_valid()) {
                ...
                if (isset($_POST['submit'])) {
                    ...
                    $objPackageActivity = new student_package_activity();
...
// for skilltype
                ...
                output::redirect(config::url("account/studentes/studentpackage/list/{$student_uid}/"));
            }
        } else {
            output::redirect(config::url("account/studentes/studentpackage/list/{$student_uid}/"));
        }
...
// function for skill type
    ...
    private function getActivityContent($student_package_uid=null, $skill_type=null) {
        if ($student_package_uid != null && $skill_type != null) {
            $html = $this->getschooladminPackageLearnableLanguagesForSkillType($student_package_uid, $skill_type, 'getActivity');
            return $html;
        }
        return ' ';
    }

    private function getschooladminPackageLearnableLanguagesForSkillType($student_package_uid=null, $skill_type=null, $endMethod=null) {
        if ($student_package_uid != null && $endMethod != null) {

            $objPackage = new student_package($student_package_uid);
            $objPackage->load();
            $class_package_uid = $objPackage->get_class_package_uid();
            ...
            if (count($result) > 0) {
                foreach ($result as $row) {
                    ...
                    $arrDiv[] = '<div id="Alanguage-' . $row['uid'] . '_' . $skill_type . '">
									' . $this->getschooladminPackageYearsForSkillType($row['uid'], $student_package_uid, $skill_type, $endMethod) . '
								</div>';
                }
            }
// return  make::tpl('body.admin.student_package.tabs.inner')->assign(
            return make::tpl('body.admin.package.tabs.inner')->assign(
                    array(
                        'tabs.lis' => implode('', $arrLi),
                        'tabs.divs' => implode('', $arrDiv)
    ...
    private function getschooladminPackageYearsForSkillType($language_uid=null, $student_package_uid=null, $skill_type=null, $endMethod=null) {
        if ($language_uid != null && $student_package_uid != null && $endMethod != null) {
            $package_uid = $this->parts[5];
            ...
            if (count($result) > 0) {
                foreach ($result as $row) {
                    ...
                    $arrDiv[] = '<div id="Ayear-' . $language_uid . '_' . $row['uid'] . '_' . $skill_type . '">
									' . $this->getschooladminPackageUnitsForSkillType($language_uid, $student_package_uid, $row['uid'], $skill_type, $endMethod) . '
								</div>';
                }
            }
            return make::tpl('body.admin.package.tabs.inner')->assign(
    ...
    private function getschooladminPackageUnitsForSkillType($language_uid=null, $student_package_uid=null, $year_uid=null, $skill_type=null, $endMethod=null) {
        if ($language_uid != null && $student_package_uid != null && $year_uid != null && $endMethod != null) {
            $package_uid = $this->parts[5];
    ...
    private function getActivity($language_uid=null, $unit_uid=null, $skill_type=null, $endMethod=null) {
        $objstudent_packageActivity = new student_package_activity();
        if ($unit_uid != null && $skill_type != null) {
            ...
            $query.=" WHERE ";
            $query.=" RPA.package_uid = '{$schoolpackuid}'";
            ...
            if (count($result) > 0) {
                foreach ($result as $row) {
                    ...
                    $html.=$objstudent_packageActivity->checkExist($package_uid, $language_uid, $row['uid']) . '/> ';
                    $html.=$row['name'];
                    $html.=' </label></p>';
                }
            }
