...
class Subscribe extends Controller {
	...
	protected function show_school_stage_1() {
		...
		list($arrErrors, $arrMessage, $form1) = $objSchoolStage1->subscribeValidation('subscribe_mike/school/');
		...
		if (count($arrStageInfo) > 0) {
			 ...
			 * Fetch the body content
			 */
			$body = new xhtml('body.subscribe.school.stage1.mike');
			$body->load();
			$body->assign(
				array(
	...
	protected function show_school_stage_2() {
		...
		list($arrErrors, $arrMessage, $form2) = $objSchoolStage2->subscribeValidation('subscribe_mike/school/');
		/**
		 * Fetch the page data from the database for this given locale
		 */
	...
	protected function show_school_stage_3() {
		 ...
		 */
		list($arrErrors, $arrMessage, $form3) = $objSchoolStage3->subscribeValidation('subscribe_mike/school/');
		/**
		 * Fetch the page data from the database for this given locale
		 */
		...
		if (count($arrStageInfo) > 0) {
			 ...
			 * Fetch the body content
			 */
			$body = make::tpl ('body.subscribe.school.stage3.mike');
			$body->assign(
				array(
					'errors'					=> $arrMessage,
					'type'						=> 'school',
	...
	protected function show_school_stage_4() {
		...
		if (count($arrStageInfo) > 0) {
			 ...
			 * Fetch the body content
			 */
			$body = make::tpl ('body.subscribe.school.stage4.mike');
			$body->assign(
				array(
					'type'						=> 'school',
					'translate.back_to_homepage'=> '',
