...
class component_callback {
	...
	private static $form		= array (
		'name'		=> array ('error'=>'', 'highlight'=>false, 'value'=>'Name'),
		'phone'		=> array ('error'=>'', 'highlight'=>false, 'value'=>'Contact Number')
	);
	...
	public static function process ($environment='') {
		 ...
		 */
		if(count($_POST) > 0 && (isset($_POST['submit_callback']) || isset($_POST['submit_callback_x']))) {
			/**
	...
	private function xhtmlresponse () {
		if(self::$emailed) {
			$_SESSION['plugin.callback.message'] = true;
		} else {
			/**
			 * Load the appropriate template
			 */
			$xhtml = new xhtml ('plugin.callback');
			$xhtml->load();
			...
			} else if(isset($_SESSION['plugin.callback.message'])) {
				self::$message = '<p class="success">'.self::$success.'</p>';
				unset($_SESSION['plugin.callback.message']);
			}
